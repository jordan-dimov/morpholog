...
`morpholog inspect guarantees carbon_credit_provenance` lists what this model forbids - one entry per invariant, naming the forbidden state for the `not(...)` rules (`retirement_terminal`, `obligation_not_both_satisfied_and_breached`). It is the static companion to `explain`: the model declares what is impossible, and `explain` says why a specific transition was refused.
