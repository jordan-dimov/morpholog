...
Standard library only; no packages to install. It uses the `morpholog` on your `PATH` (set `MORPHOLOG_BIN` to point elsewhere, e.g. `target/release/morpholog`). It prints each lifecycle step, the intent it delivered, and a closing list of the interface friction it hit.
