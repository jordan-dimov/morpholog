...
It also forced the read side, twice. To settle against a price it did not itself mint - the position of any resumed process or separate service - an embedder has to *read current governed state back*; that friction forced `morpholog inspect claims --predicate`, the targeted claim query. Then decoding those positional reads by declared field name turned out to be friction of its own: this script and the first real external embedder independently hand-rolled the same zip-by-declared-order-with-arity-guard helper, and two reinventions of one decode is exactly the bar - it forced `--named`, the read-side mirror of `--args-named`, and the hand-rolled helper is deleted. The schema provisioning likewise goes through `morpholog init` now - the embedded-schema path a binary-only deployment uses. The residual friction, printed at the end, is that selection stops at predicate granularity: an argument-level filter waits for an example with a book big enough to force it.
