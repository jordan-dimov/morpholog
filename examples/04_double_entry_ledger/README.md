...
5. **Account-balance derived claim keyed on date.** This example hosts `TrialBalanceRow` (one row per account, debits-minus-credits), which is the basic derived view. A balance-as-of-date variant would key on `(account, date)` and aggregate only lines posted on or before the given date; deferred until a real example needs the date filter (the current as-of CLI flag already answers the related "as of which transition?" question against `TrialBalanceRow`).
