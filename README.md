...
The `.morph` parser arc is mid-stream. A `.morph` source file can carry: the `program` header, `predicate` declarations, full expressions (atoms, arithmetic, comparators, boolean composition, `exists`/`forall`/`sum`/`value`/`in`, date and subject literals), `invariant` declarations, and `transformation` declarations with `require` / `bind` / `let` (including `let name = new Subject()`) body statements - indentation-based block syntax. `morpholog parse <file>` parses these end-to-end. The remaining four statement keywords (`admit`, `retract`, `emit`, `for`) and derived-claim declarations are the next parser increment.
...
The **surface syntax** arc is the current major investment. A parser turns `.morph` source into the IR the kernel already runs. The natural reader of a Morpholog programme is a domain expert (a controller, an auditor, the person who actually understands the business rules) - not a Rust developer. Predicates, expressions, and invariants parse today; transformations and derived claims land next (see [`docs/roadmap.md`](docs/roadmap.md) for the per-PR breakdown). After the parser arc completes, the next gap is **legibility tooling** for that same audience: `morpholog inspect` subcommands that answer "what does this programme forbid?" before "what does it enable?" - exclusion matrices, transformation graphs, subject-flow profiles, all derived from static analysis of parsed programmes.
