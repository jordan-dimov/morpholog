...
- [**Double-entry ledger with period close**](examples/04_double_entry_ledger/) - the accounting equation enforced as an invariant; period close as an admission gate; closed periods corrected by restatement that preserves the original record.
- [**Trial balance as a derived claim**](examples/05_trial_balance/) - the first read-side projection. A *governed view* defined alongside the invariants that govern the underlying claims; enumerated on demand by the same kernel that admits them. As-of evaluation falls out for free. Attaches to the Example 4 program rather than declaring its own.
- [**Actor authority**](examples/06_actor_authority/) - the smallest model that consults the proposing actor of a transition. A domain transformation whose argument list contains only domain payload, with the actor reached from transition context via `$actor`. Authority is granted, revoked, and lost without ever invalidating the historical record of past decisions.
...
Not in the box yet: a parser (programs are constructed as Rust IR; the CLI accepts built-in programs only); a worker supervisor with circuit breakers and an HTTP-aware deliverer (a polling worker exists in `morpholog-outbox` with a `StdoutDeliverer`, but driving multiple workers under restart-with-intensity is PR-after-next); approval limits and predicate-pattern matching (unconditional actor authority works today; comparison-based limits await the example that needs them); user-supplied program loading; materialised derived claims. Each lands when a worked example forces the shape.
...
- **Approval limits with decimal comparison.** Unconditional actor authority works today - the asserted record carries who admitted it, and a `require` clause can gate admission on whether the proposing actor holds the relevant authority. The next shape is *quantitative* authority: "may approve invoices up to $50,000". That forces a decimal comparison primitive in the expression language. Predicate-pattern matching (`may approve any claim of this shape`) is the further shape after that. Both land when an example forces them.
...
- Worked examples: [`examples/01_settlement_netting/`](examples/01_settlement_netting/), [`examples/02_revenue_restatement/`](examples/02_revenue_restatement/), [`examples/03_claim_standing/`](examples/03_claim_standing/), [`examples/04_double_entry_ledger/`](examples/04_double_entry_ledger/), [`examples/05_trial_balance/`](examples/05_trial_balance/), [`examples/06_actor_authority/`](examples/06_actor_authority/).
