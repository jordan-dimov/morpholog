...
**What about GDPR's right to erasure, if nothing is ever deleted?** Subjects are opaque identifiers by design. Keep personal data in an ordinary, erasable store keyed by subject id; erasure deletes the mapping, and the immutable history holds only meaningless identifiers. Redaction within history itself (per-subject crypto-shredding) is a recognised future direction.
