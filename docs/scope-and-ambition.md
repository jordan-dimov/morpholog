...
| `=`, `!=` (infix) | `Expr::Eq` (Expr, Expr), `Expr::Neq` (Expr, Expr) | Both operate on full expressions and are symmetric: `a + 1 != b` is as legal as `a + 1 = b`. Kind-agnostic value comparison, but `==`/`!=` are kind-strict (no silent coercion). |
