...
- **PR P2b: bounded forms and remaining literals.** `exists x: body`, `forall x in coll: body`, `sum(target | body)`, `value(target | body)`, `in` operator, date literals, `DateLe`. The harder bounded-comprehension territory. Two design constraints worth pinning before the PR starts: (1) the `in` keyword carries two roles - membership comparator (`x in xs`) and binding source (`forall x in xs`, `for x in xs`) - and the grammar must make the binding form unmistakable, never inferred from context. (2) The `sum(target | body)` form is the locked syntax (set-builder notation, mathematical, compact); Python-style generator expressions are explicitly out of scope - Morpholog should feel like business mathematics, not embedded Python.
- **PR P3+: invariants, transformations, derived claims.** Expression parser embedded into programme-level constructs. Statement syntax (`require`, `bind`, `let`, `admit`, `retract`, `emit`, `for`). In some order, once expressions are fully stable.
