...
- **PR P2b: bounded forms and remaining literals.** `exists x: body`, `forall x in coll: body`, `sum(target | body)`, `value(target | body)`, `in` operator, date literals, `DateLe`. The harder bounded-comprehension territory.
