...
- **Concurrent proposals into one contended aggregate anti-scale.** The `contend` scenario shows the SERIALIZABLE 40001 retry rate climbing roughly linearly with worker count while total throughput falls: posts that read a shared footprint and write into it serialise under SSI. The product-shaped response is to partition contended state (per book / account / period) or batch, not to optimise the substrate. This also supplies the measured 40001 rate the substrate question below requires.
