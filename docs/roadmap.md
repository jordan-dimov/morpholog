...
- **PR P3b: transformations + statement syntax.** The seven statement forms (`require`, `bind`, `let`, `let new Subject()`, `admit`, `retract`, `emit`, `for x in coll: <stmt-list>`) and the `transformation <name>(<params>): <stmt-list>` header. Date-comparison surface form lands here if any worked example body needs it.
