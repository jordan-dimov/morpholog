...
- **PR P3+: invariants, transformations, derived claims.** Expression parser embedded into programme-level constructs. Statement syntax (`require`, `bind`, `let`, `admit`, `retract`, `emit`, `for`). Date-comparison surface form lands here when an invariant body uses it. In some order, once expressions are fully stable.
