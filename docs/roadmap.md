...
- **PR P3b2 (landing): state-mutating statements + iteration.** `admit`, `retract`, `emit`, plus `for x in coll: <indented body>` (introduces nested layout). Reuses the P3b1 `claim_pattern` helper across all four verbs. After this, four of six worked examples parse end-to-end via `morpholog parse`; the remaining two stop only at `derived` (P3c). Date-comparison surface form deliberately deferred to its own focused PR.
- **PR P3c: derived claims.** `DerivedClaim` shape with keys, values, and domain. Round-trip property tests (`format_program -> parse_program`) land here when the full surface is parseable. After P3c, all six worked examples parse end-to-end.
