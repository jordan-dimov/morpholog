...
- The tamper-evidence envelopes: `verify` (`verify_report`: a replay verdict beside a tree verdict, plus the optional views and witnesses axes), `checkpoint` (`checkpoint_outcome`), and evidence-pack `export` (`evidence_pack`) / `verify` (`tree_verification`, including the `malformed_pack` verdict; `pack_verification_report` when the witness axis is requested).
