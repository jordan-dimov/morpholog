...
- Signed tree heads: `checkpoint --signing-key <pkcs8.pem> --key-id <id>` (key pairs from `keygen`) signs the new tree head, so a checkpoint / pack checkpoint carries an optional `signatures` array (`tree_head_signature`: `key_id`, `purpose`, `public_key`, `signature`), and a signature that does not verify over its tree head is the `signature_invalid` verdict. Verification proves a signature is genuine over its tree head - making an externally held anchor attributable; whether the signing key is *authorised* (an admitted `AuditSigningKey` claim) is a later layer, so for now trust in a public key is established out of band.
