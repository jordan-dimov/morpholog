...
Accepts the subset of transformations whose every parameter resolves to one of `Subject`, `Decimal`, `Date`, `Timestamp`, `Duration`, or `Bool`. Where the schema cannot give an unambiguous scalar kind, the codec refuses with an error pointing at `--args`. Per-kind behaviour:
