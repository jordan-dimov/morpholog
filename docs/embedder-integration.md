...
- Compliance mode: `verify --require-signatures` / `evidence verify --require-signatures` fails an otherwise-intact tree that carries any unsigned checkpoint (`signature_required`). Signing is opt-in by default; this is the verifier's policy, applied over the intrinsic verdict.
- Exit-code semantics for `propose`, `explain`, `verify`, and `evidence verify` (a divergence, tamper, or malformed pack is a decided verdict on stdout at exit one, not an operational failure).
...
The read, commit, and tamper-evidence surfaces are all under this contract now: `verify` (the `views` verdict included), `checkpoint`, and evidence-pack `export` / `verify` (prefix and window alike) are pinned in `schema --result`, byte-pinned in the golden set, and emitted as typed methods by `generate python-client` (`verify`, `checkpoint`, `evidence_export`, `evidence_export_window`, the offline `evidence_verify` / `evidence_verify_window`) - no bridge between them and the embedder. The `evaluate` score reports (single, batch, and train/test-split forms) are pinned in `schema --result` and the golden set too - they are what a discovery harness consumes - with a generated client method deferred until a harness asks for one. The legitimacy work builds on that contract rather than reopening it.
