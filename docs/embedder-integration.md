...
- Index provisioning: `provision indexes <file.morph>` reconciles the partial expression indexes a programme's loads and compiled invariants seek on, the columns a check is bounded to a case by included, whichever route it runs, and analyzes the claims table after building any, so the planner has statistics over the new expressions from the first proposal (`--dry-run` prints the plan, `--prune` drops managed indexes no programme requires; a conflict under Morpholog's own index name exits non-zero for an operator). Correctness never depends on it: the loads and the checks are right without any index, only slower, and a load without its index reads its predicate whole and locks accordingly.
