...
The read, commit, and tamper-evidence surfaces are all under this contract now: `verify`, `checkpoint`, and evidence-pack `export` / `verify` (prefix and window alike) are pinned in `schema --result`, byte-pinned in the golden set, and emitted as typed methods by `generate python-client` (`verify`, `checkpoint`, `evidence_export`, `evidence_export_window`, the offline `evidence_verify` / `evidence_verify_window`) - no bridge between them and the embedder. The legitimacy work builds on that contract rather than reopening it.
