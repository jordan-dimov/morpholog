...
**`attestation`** records how the actor identity was established. Gateway mode - `{"mode":"gateway","authenticated_by":"<role>","authenticated_by_oid":<oid>}` - means the PostgreSQL-authenticated login role of the proposing connection asserted the actor; it proves who asserted, never that the named actor authorised anything. `authenticated_by_oid` is that role's OID, which tells one incarnation of a name from another: a role dropped and created again under the same name gets a new OID, and inherits every grant made to the name. It is local to the cluster, not a permanent identity, and rows written before it was recorded carry none. A session whose role was dropped or recreated under it cannot propose at all. The value is resolved by the runtime from the connection itself, never supplied by the caller, and it is covered by the Merkle leaf: rewriting or stripping it after the fact breaks `verify`. Rows written before attestation existed carry none and keep their original leaf encoding, so existing checkpoints and packs stay valid. Operational notes for that boundary: a generated client from before the field existed refuses the first attested row by design (the drift tripwire - regenerate the client), and an offline verifier binary from before the field existed recomputes the wrong leaves for attested rows and reports a root mismatch on genuine history - upgrade the verifier before believing it.

**`parameters`** names the row's own signature: the transformation's parameter names in declaration order, one per argument, stamped by the runtime at commit. A row is therefore readable with no client-side vocabulary at all, and stays so after the act that wrote it is retired from the programme - zip `parameters` with `arguments` and the call reads as it was made, including what it did *not* contain, which is the claim an audit product most needs to keep honest. The names are inside the Merkle leaf (the self-describing encoding, content-derived like the original and the attested ones), so a renamed parameter in an exported pack is tamper, not a display quirk. Rows written before names were stamped - before migration 014, which arrived in v0.0.11 - carry no `parameters` and hash as they always did; the migration never backfills them, and the same one-way boundary as attestation refuses any new unstamped row. `--named` does not touch `arguments`: the names are historical row data, not something the programme supplied, so the bare row is already self-describing and no second keyed representation is emitted.
...
- The audit row's `invariants_checked`: every active invariant the transition was admitted under, each discharged because the transition's effective delta could not affect it, because every affected case satisfied it, or because the whole invariant was evaluated and held (admission is case-local; see `docs/runtime-semantics.md`).
...
- The tamper-evidence envelopes: `verify` (`verify_report`: a replay verdict beside a tree verdict, plus the optional views and witnesses axes), `checkpoint` (`checkpoint_outcome`), and evidence-pack `export` / `verify-pack` (`pack_verification_report` around a `tree_verification`, including the `malformed_pack` verdict).
...
- `audit verify-pack` prints a `pack_verification_report`: `{verdict, role_rebindings}`, plus `witnesses` when asked for. Before this release it printed the bare verdict unless witnesses were requested.
- Complete-prefix packs are written one line at a time (`pack_format_version` 4). Line 1 is a `prefix_pack_manifest`; the next `checkpoint_count` lines are `checkpoint`s; then exactly `tree_size` lines, each an `audit_row`, in log order, and nothing after. Every line ends in a newline. `audit export` streams it and `audit verify-pack` reads it the same way, so neither holds the history in memory: the verifier keeps one row, the Merkle frontier, the checkpoints, the authorised signing keys and the role names it has met. The pack repeats each value in `arguments` and `asserted_claims`, because the leaf hashes both as written; gzip removes that repetition, and `verify-pack` and `evaluate --pack` read a gzip-compressed pack as it is (`morpholog audit export | gzip > pack.ndjson.gz`). A pack in the single-document form (`pack_format_version` 1) from an earlier release still verifies, and `evaluate --packs` reads every form. The generated client's `audit_export(path)` writes the pack beside `path` and moves it into place only once the export has succeeded, returning the manifest; it no longer returns the pack itself.
- Role rebindings: `audit verify` and `audit verify-pack` report, as `role_rebindings`, each login-role name seen under a new OID in the rows they checked - a role dropped and created again. It is a finding, never a failure, and never changes the verdict. It is `evaluated` only over rows an intact verdict established (for `audit verify`, the rows up to the latest checkpoint), and `not_evaluated` otherwise. `scope` says what those rows are: a complete prefix, a window, or a selective pack, which shows only the rows it discloses, so a change is reported between the last and first transitions *observed*, not necessarily the last and first in the log. `rows_without_oid` counts rows that could not show a change.
- Witnessed checkpoints: `audit checkpoint --witness rfc3161:<url>` (repeatable; or `audit witness --tree-size N --witness ...` for a head recorded earlier) has a public RFC 3161 timestamp authority countersign the new tree head, and stores the authority's exact response on the checkpoint as a `witness` (`scheme`, `proof` base64, `submitted_to`) - nothing derived from it is stored, so what a witness proves is always read from the proof itself. The bytes witnessed are the head's own typed, length-delimited **witness payload** (its own frozen type, distinct from the signing payload). A signature says the operator's key vouched for a head; a witness says the head **existed no later than the authority's time** - it never says anything about business completeness, and never subtracts from the tree's own verdict. Submission happens after the commit, outside any transaction: every authority named is attempted and each response stored as it arrives, the checkpoint is recorded and printed whatever they do, and any failed submission exits one naming each failed authority and the one command that retries exactly those. A response that is not over this head is refused before storage. Verification is the verifier's, with its own trust: `audit verify --trusted-tsa-file <cas.pem>` judges every stored witness in the chain (the `witnesses` field of `verify_report`, present whenever any checkpoint carries one) and `audit verify-pack --witnesses` (or `--trusted-tsa-file`, which implies it) adds `witnesses` to the `pack_verification_report`. Each `witness_verdict` is `verified` (a certification path from the token's signer to an anchor you supplied validates **at the attested time** - names chain, signatures verify, every certificate was valid then, each issuer is an authority whose constraints admit the path; the signature names its certificate, is the token's only one, and that certificate carries the critical timestamping usage alone; revocation and policies are not checked, so choose anchors accordingly), `untrusted` (sound, but no such path to an anchor you named; `detail` says where it failed), `unverified` (sound, no anchors supplied), `unsupported` (this build cannot check its signature primitive - never reported verified, never a failure), or `invalid` (it does not vouch for this head); `earliest_attested_at` rests on verified witnesses only. Only `invalid` fails the command. Packs of every kind carry the witnesses of the checkpoints they already carry.
- Compiled invariant checks: when `check -v` reports `invariant checks: compiled`, a proposal loads only the rows its transformation body can observe (the predicates it consults, keyed by what its arguments fix) and checks each invariant in the database over the cases the written change touches; otherwise the interpreter checks them in memory, over the same keyed loads plus every claim the invariants mention. Same decisions, same envelopes, same audit record either way; only the cost differs. Trace and explain-on-reject always run the interpreter.
- Index provisioning: `provision indexes <file.morph>` reconciles the partial expression indexes a programme's loads and compiled invariants seek on, including the columns that bound a check to a case, whichever route it runs, and analyzes the claims table on every run that applies, so the planner has statistics over the expression indexes from the first proposal (`--dry-run` prints the plan, `--prune` drops managed indexes no programme requires; a conflict under Morpholog's own index name exits non-zero for an operator). Correctness never depends on it: the loads and the checks are right without any index, only slower, and a load without its index reads its predicate whole and locks accordingly.
...
Those rows are the ledger posting, whose only gate reads an empty predicate. The shape that contends is a gate that reads the predicate it then writes, a capture: sixteen writers admitting distinct items into one predicate of five thousand rows, each first requiring its item absent:

| Reads | Retries per commit | Commits per second |
|---|---|---|
| whole predicate (no indexes provisioned) | 8.5 | 60 |
| keyed by the item, indexes provisioned | 0.08 | 5,500 |

A keyed read remembers only the index range it searched, so disjoint items stop conflicting. Without its index a keyed read scans the predicate and computes a key per row, which is slower than the old whole read (about 45 against 60 commits a second here) and locks the same relation, so run `provision indexes` for every programme, interpreted or compiled. On the interpreted route the invariants' predicates are still read whole, so a programme whose invariants mention the predicate a gate writes keeps the first row's retries until that is narrowed.
