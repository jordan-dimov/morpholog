...
morpholog generate views x.morph | psql "$DATABASE_URL"
```
...
- **Applying and evolving.** Apply the whole script in one shot - it is transactional, so a failure rolls back rather than leaving a half-built surface. Across model versions, **appending a predicate field is a compatible `CREATE OR REPLACE`** (the new column appends at the end); **renaming, removing, reordering, or retyping a field is not** - that needs a manual view migration or a `DROP VIEW` first, because `CREATE OR REPLACE VIEW` forbids changing an existing column's name, position, or type.
