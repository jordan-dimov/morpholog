...
The round-trip story's original gap - stringly-typed intents - is now closed. `IntentDecl` mirrors `PredicateDecl`: every `emit X(args)` must target a declared intent, and `morpholog check` validates emit name, arity, and arg kinds the same way it validates a claim against its predicate. A misspelled intent name is a validation error, not a silent dead outbox partition. The KYC sanctions-screening example forced it. What remains open is narrower: an `inspect intents` CLI view to surface the declared intent schema to a non-Rust deliverer (the declarations exist; the read-side view does not yet), and per-intent delivery-target metadata, which is outbox-worker configuration rather than vocabulary.
