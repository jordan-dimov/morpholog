...
Status: doctrine doc tracking realised implementation. The substrate (PR #32), single-row processor with compensation lease (PR #33), and polling worker + smart sleep + StdoutDeliverer (PR #34) are in tree. The remaining slice is the supervisor + circuit breaker + HttpDeliverer. The spike test at [`crates/morpholog-postgres/tests/outbox_spike.rs`](../crates/morpholog-postgres/tests/outbox_spike.rs) is subsumed by the processor's end-to-end tests and the outbox crate's worker tests, but remains as a historical reference. Deletion of this doc happens once `docs/forced-by-examples.md` carries the full retrospective.
...
2. **(landed in PR #33) Single-row processor + `DeliveryOutcome` + `Deliverer` trait + compensation lease.** `process_one_outbox_row` drives one full cycle. Added the `compensation_in_progress` status and three lease helpers (`begin_compensation`, `complete_compensation`, `mark_compensation_failed`) to close the duplicate-compensation race the PR 1 substrate did not solve on its own. The `Deliverer` trait uses RPITIT with `Send + Sync` baked in. `CompensationSpec` uses a boxed args closure so callers can pass `None` without an inference workaround.
3. **(landed in this PR) Polling loop + per-target worker + first concrete deliverer.** New `morpholog-outbox` crate: `process_available_outbox_rows` drain (loops `process_one_outbox_row` until `NoRowAvailable`); `OutboxWorker<D, C, R>` generic over `Deliverer`, `Clock`, and `JitterRng` so production uses real time + real randomness while tests inject `MockClock` + `FixedJitter` for deterministic timing assertions; shutdown via `tokio::sync::watch::Receiver<bool>`; smart sleep clamps the post-drain interval to the soonest `next_attempt_at` (via the new `earliest_pending_retry` helper) so Transient retries fire when they say they will rather than at the next base-interval boundary; `StdoutDeliverer` as the canonical first concrete impl. Still no supervisor and no circuit breaker.
4. **(then) Supervisor + circuit breaker + HttpDeliverer.** `JoinSet`-based restart-with-intensity across multiple `OutboxWorker` tasks. `failsafe-rs` per-target breaker. `HttpDeliverer` that POSTs intents to a configured URL and routes 5xx/connection errors to `Transient`, 4xx (except 408/429) to `NonRetryable`, 2xx to `Delivered`.
