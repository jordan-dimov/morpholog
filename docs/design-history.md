...
**Landed:** the surface gained one position-aware walk for its observation passes, with substitution and the const subset keeping their own match; the lexer's vocabulary is one list that yields the enum, the rendering, and the reserved-word map; the validator's two depth walkers became one, its six duplicate-name blocks one helper, the lint's re-walked temporal evidence one walk, the discipline clauses one iterator; the pack module's shared operations were extracted by responsibility rather than behind a pack trait; and the rule went into CONTRIBUTING: structural questions use the shared walk, semantic authorities match the IR directly. Two candidates were measured and declined. Folding the four-way audit pager into one query with presence flags keeps the row comparison out of the index condition under a generic plan, so every page would walk the index from its start; sixty duplicated SQL lines are cheaper than that on legitimacy-grade data, and a null-cursor sentinel would make a row at the sentinel invisible, the wrong trade in code whose job is to detect tampering. The outbox worker's clock and jitter traits looked like test scaffolding tokio's paused time could replace, but the clock also supplies the wall time the lease timestamps are written with, so removing it is a test-architecture decision, not a subtraction.
