...
**Landed:** two new public items on `morpholog-core`. `analysis::transformation_param_kinds(program, name) -> Result<Vec<(Var, ParamKind)>, AnalysisError>` is the kernel piece - the resolved input contract. `schema::transformation_arg_schema(program, name) -> Result<serde_json::Value, AnalysisError>` is one adapter over it - JSON Schema (Draft 2020-12), Subject as opaque-uuid string, Decimal as strictly-patterned string (no leading zeros, no bare `+`, no trailing dot), Date as ISO civil date, Bool as boolean, Collection as untyped array, Polymorphic and Unconstrained as typeless properties whose description carries the state. The boundary is deliberate: **the kernel exports the inferred input contract, not a client protocol.** A future embedder wanting an OpenAPI component or a Python dataclass renders the same `ParamKind` result a different way; the truth of the analysis is exported once, the encodings are adapters.
...
**Four-state `ParamKind` (Concrete / Polymorphic / Unconstrained / Ambiguous), not `Option<PredicateArgKind>`.** Collapsing any pair loses a distinction the embedder presents differently. A `Polymorphic` parameter flows through `PredicateArgKind::Any` slots (the declaration-time escape hatch) and the embedder should accept input but flag that the kernel cannot narrow it; an `Unconstrained` parameter is never observed in any kind-bearing position and the embedder should surface it as dead-or-broken; an `Ambiguous` parameter is observed at two or more distinct concrete kinds across cloned scopes the checker hides (typically `Or` branches the runtime picks between), and the embedder should render it disjunctively (JSON Schema `anyOf` in the adapter that landed). The first review of this PR caught a bug here: an earlier collector refined into one `InferredKind` per variable and silently dropped a conflicting later observation, which would have made the schema lie about transformations that legitimately accept disjunctive input shapes. The fix accumulates a *set* of observed kinds per variable and projects multi-kind sets to `Ambiguous(Vec<PredicateArgKind>)` in deterministic declaration order. Reporting one concrete kind is a lie; refusing to emit a schema is too strict; collapsing to `Polymorphic` hides the diagnostic - four states are honest.

**Eq / Neq cross-refinement deliberately omitted.** The walker does not pin a bare-variable operand of `Eq` / `Neq` from a literal on the other side. The simpler walker is ~115 lines; the cross-refining version was ~210 with a helper that re-implemented kind inference for every `ValueExpr` shape. Real models flow parameters through claim / intent arg positions; the trade-lifecycle integration test confirms total inference without cross-refinement. An example that genuinely needs it becomes the witness that forces it back in - matching the kernel-primitive-plus-forcing-example doctrine.
