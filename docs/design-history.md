...
**Landed:** `PgProgram`, the adapter's own programme object: the validated core programme beside the PostgreSQL execution plan built once from it. The SQL compiler that had lived behind a test gate since rung 1 now runs at programme load; every invariant compiles or none does, and the interpreter runs the whole programme when one is outside the fragment. `check -v` reports the outcome - `invariant checks: compiled`, or `interpreted` with each refusing invariant and the construct that kept it out - so an author knows before proposing anything. No production path reads the decision yet; the review settled that the object lives in the adapter and never in the substrate-neutral core type, that state loading gains a second authority on the compiled route (body reads only), that the acceptance record is split from the delta write before the compiled path is wired, that diagnostics stay interpreted, and that index provisioning lands before the production switch so no `main` carries a compiled default without the mechanism that makes it pay.
