...
### Admission is case-local

**Forced by:** #277 rung 2b's second step. The three-way benchmark attributed the spike's flat scaling and narrowed contention to the case-bound stage 2; making that the production rule on the compiled route alone would have let inherited dirt block a transition or not depending on whether its invariants happened to compile. The review settled the rule and where it lives.
...
### The public Rust API is a committed snapshot

**Forced by:** three reviews in one day (#338, #341, #342) caught the same mistake - a refactor that removed or reshaped public Rust items presented as an internal change - and nothing in the gate would have. #345 asked for a mechanical finding instead of a review discovery.

**Landed:** the public surface of the library crates (core, postgres, cli, witness, surface, outbox) committed as text under `api/`, rendered by `cargo public-api` from rustdoc JSON with all features, and checked in precommit and CI. A change to the surface is an ordinary diff of `api/` in the PR that makes it; additions are as visible as removals, so an accidental new `pub` is caught like an accidental removal. The snapshots describe the API at the commit that lands the gate, not at a release. The crate list is explicit rather than discovered. The nightly toolchain and the tool version are pinned in the script alone, so stable and the declared floor remain the compiler contract, and the check never skips: it needs nothing outside the repository.

**The instrument chosen, and the one not.** `cargo-semver-checks`, the issue's first suggestion, was measured against v0.0.10 and reported six findings across three crates, but missed the month's largest break: every propose facade moving from `&CompiledProgram` to `&PgProgram` in #353, because it does not check parameter types. It also errors on crates that did not exist at the baseline. The snapshot caught everything it found, showed #353 as a two-line diff, and rendered identically on rerun. Its major and minor labels buy nothing at 0.0.x; marking the public enums `#[non_exhaustive]`, which four of its findings pointed at, changes which API changes are compatible and is a separate decision. Break-checked: a changed signature on a re-exported type and an added public method each fail the check with the one line named; a public item in a private module correctly does not appear; a stale snapshot for an unlisted crate and a wrong tool version each fail with the remedy.


### The adapter runs on one time library

**Forced by:** #346. The kernel's domain timestamps were jiff while every operational instant in the adapter, the outbox and the CLI was chrono, so the workspace carried two clock types and converted between them by hand. Once `wire_time` had frozen the bytes, the swap could be proved rather than trusted.
