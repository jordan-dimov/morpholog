...
**Landed:** `settle_trade` may now run more than once per trade, and `TradeSettled` became a settlement *slice* (several per trade). The per-settlement `settled_qty <= captured_qty` invariant became the cumulative `sum(q | TradeSettled(trade, q, _, _)) <= captured_qty` - the running total across slices is what stays in bounds, exactly the aggregate-cap shape the insurance example pins against a policy. A companion `settlement_id_identifies_one_settlement` uniqueness invariant pins the structural property the sum depends on, mirroring insurance's `settlement_id_uniquely_identifies_payment`: without it a slice could be double-counted or two settlements could hide under one id. No new kernel primitive - `sum`, `<=`, and claim uniqueness all existed; this is cross-example pattern reuse, the cumulative cap moving from insurance into the trade domain. The cap stays an invariant, not a require gate (consistent with how the example already enforced the single-settlement bound), so an over-total slice is refused at commit by `settled_quantity_within_captured`.
