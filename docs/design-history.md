...
**Landed:** one commit per observable difference. An instant has one spelling on the wire - RFC 3339, UTC, `Z`, the shortest of zero, three, six, or nine fractional digits that loses nothing - owned by one adapter module that every serialized and rendered timestamp passes through and pinned by vectors, so the clock type behind it can change without moving a byte; the score report's golden moved to the spelling the scorer really emits, and nothing else did. The precision rule was widened from the database's microseconds in review, because the witness axis and a user-supplied boundary can carry more than the database stores, and a canonical boundary must not be a lossy one. The CLI's own parsers stay where they are: freezing the bytes was the point, not every internal parser. An incomplete audit prefix is its own typed error, raised by the one loader the three exporters share, and named factually rather than as tampering, since deletion, loss, and hostile edit are interpretations of one fact. A `where` clause is read by one resolver on both surfaces, with the session's error code unchanged. And the manual-schema intent ledger records, as a test, what would be lost if the hand-written schema were replaced by a generated one.
