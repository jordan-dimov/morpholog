...
**Three-state `ParamKind` (Concrete / Polymorphic / Unconstrained), not `Option<PredicateArgKind>`.** Collapsing the two informationless states loses the distinction the embedder presents differently: a `Polymorphic` parameter flows through `PredicateArgKind::Any` slots (the declaration-time escape hatch) and the embedder should accept input but flag that the kernel cannot narrow it; an `Unconstrained` parameter is never observed in any kind-bearing position and the embedder should surface it as dead-or-broken. Same JSON-Schema shape today (typeless property with a description), but the embedder's UX renders them differently.
