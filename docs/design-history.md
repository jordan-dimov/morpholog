...
### Opaque newtypes for the kernel's identifiers: `Subject`, actor, `Var`
...
**Landed, one opaque newtype per slice** - `From` / `Display` / `as_str` only, no `Deref` / `AsRef` / `Borrow`, so call sites stay explicit: `Subject` for the opaque subject primitive (`EvalValue::Subject`, `Value::Subject`), `#[serde(transparent)]` keeping the `{"type":"subject","value":...}` wire shape byte-identical; then `Transition.actor: Subject`, which *deleted* the runtime `matches!(actor, EvalValue::Subject(_))` guard - a non-subject actor is now unrepresentable in the kernel, and the one place a bad actor could still enter (deserialisation) carries the check in the `actor_repr` codec, which keeps the audit/CLI wire shape tagged; then `Var` for bound-variable names, unifying `Term::Var`, the `forall` / `exists` / `for` binders, `let` names, the `Bindings` keys, and the trace's variable-name fields into one type. Names in distinct namespaces (predicate, transformation, invariant, intent) stay `String`.

Like the two-sort split, this is the make-the-model-true discipline: push a doctrine the kernel only described into the types, so the compiler enforces it and the runtime stops re-checking. The `Subject` / `actor` slices were hand-propagated by a sub-agent; `Var` was done with the [refactoring playbook](refactoring-playbook.md) (compiler-worklist plus structural rewrites), which is the bar going forward.
