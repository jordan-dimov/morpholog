...
| `sum(target | body)` | `ValueExpr::Sum` | Set-builder notation. The target is any value expression consuming the body's bindings, evaluated exactly once per witness: a variable (`sum(amount | body)`), a decimal literal counting matches (`sum(1 | body)` - the chess material census forced this), or a computed quantity (`sum(probability * loss | body)` - the expected-loss recompute forced the generalisation). The target binds nothing and never changes which witnesses exist; an error evaluating it for one witness fails the whole sum. A bare `actor` or `_` target is refused by name. Type-driven: a sum of decimals is a decimal, a sum of durations is a duration (counted laytime forced this), a sum of same-unit quantities is a quantity of that unit (the cargo book forced this); mixing kinds - or units - is an error. The empty sum is the typed zero of the target's declared kinds (`0 t` over a `Decimal[t]` position, `duration(PT0S)` over a duration, decimal zero otherwise), resolved statically by the `lower_sum_seeds` pass - variables from the claim position that binds them, literals from their own kind, arithmetic through the rule matrix (`qty * factor` keeps the unit even with the factor's kind unknown); a target the pass cannot type (a lookup, a conditional, a nested aggregate) stays decimal. The extremum target is deliberately still a variable or literal - no worked example has forced a computed extremum. |
