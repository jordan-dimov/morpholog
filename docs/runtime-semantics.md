...
ValueExpr                        -- a value expression: computes exactly one value.
  ...
  ValueOf { predicate, args, extract, default }  -- unique-lookup value extraction
...
| `Pred(field: x, ..)` (named-field claim pattern; `..` = unmentioned fields are wildcards, omitted `..` = every field named) | none - lowered to the positional `Term`s at parse time | Positional patterns at arity are miscount hazards: adjacent same-kind fields let one lost underscore bind the WRONG figure silently, and kind-checking cannot catch it. Entries resolve by declared field name in any order, so a pattern survives field reordering and growth; all-named or all-positional, never mixed; `emit` resolves against the intent vocabulary, every claim-shaped site against predicates; definitions have parameters, not fields, and refuse the form. The formatter's canonical output uses the named form whenever a pattern carries a run of two or more consecutive wildcards (one wildcard is legible where it stands; `Pred(..)` is the existence check) - and `..` is an authoring-time evolution assertion, not IR metadata: formatting may erase whether a named pattern was written total or rest-open, because both lower to the same IR. Rules identity is untouched: `canonical_hash` renders positionally, so a named source and its positional twin share one hash. |
...
| `+`, `-`, `*`, `/`, `%` (infix) | `ValueExpr::Arith { op, .. }` with `op: ArithOp::Add` / `Sub` / `Mul` / `Div` / `Mod` | Standard arithmetic, exact. `+`/`-` also carry the time matrix (instant + exact span = instant, instant - instant = exact span, exact spans add and subtract, and cap via the `min`/`max` builtins; date + calendar span = date with month-end clamping, date - date = signed actual days as a decimal) and the unit algebra (same-unit quantities add, subtract, and compare (capping is `min`/`max`); a bare decimal scales a quantity via `*`/`/`; the ratio of two same-unit quantities - and of two durations - is a bare decimal, exact for terminating ratios: `excess / duration(PT24H)` gives exactly 5.5 for a 132-hour excess, while a repeating ratio carries decimal precision rather than a hidden float). Nothing produces a unit that was not already written down: no compound units, no unit-producing multiplication, and `%` stays decimal-only. The matrix is enforced twice - `NoArithRule` at authoring time, `TypeMismatch` at evaluation - and a pair with no rule (adding two instants) is refused by name, not coerced; `*`/`/`/`%` bind tighter than `+`/`-`. The operator is a field, not a per-operator variant - the value-sort analogue of `Prop::Compare` carrying a `CompareOp`. Admission gates express ratio rules in multiplied form (`a <= c*b`, not `a/b <= c`) to stay exact; `/` is reserved for read-side projections, where a rounded figure is wanted; `%` is remainder, for parity and cyclic rules (`(file + rank) % 2`). A zero divisor on `/` or `%` surfaces `EvalError::DivisionByZero`, and a result outside the exact decimal range surfaces `EvalError::ArithOutOfRange` - exactness or a named refusal, never an approximation, never a panic. Both are KERNEL evaluation errors (the adapter's `PgError::Kernel`, the CLI's error envelope), operational and without audit standing - never business rejections; a domain where extremes or zero divisors are business-possible guards them in its own rules. `sum` totals are accumulated wider than the value type, so only the final total decides representability - the answer depends on the SET of matched claims, never their iteration order. No unary minus until forced. |
| `min(a, b)`, `max(a, b)` | `ValueExpr::Call { builtin: Builtin::Min` / `Max, args }` | Floor and cap as self-delimiting functions, not infix - no extra precedence tier. Express layered limits, e.g. `min(limit, max(0, x))`. Defined over the kinds the language ORDERS: decimals, same-unit quantities, durations, dates, and timestamps. Taking the smaller of two asks the comparator's question and keeps the answer rather than the verdict, so wherever `on_or_before` is lawful `min` is too. Calendar spans are out - months and days have no common measure until a date lands them, so `span(P1M)` against `span(P30D)` has no truth to order by, and they compare for equality only. Subjects and bools are out for the same reason: nothing to minimise over. Same-kind only, enforced twice - refused by name at authoring, `TypeMismatch` at evaluation. |
| `abs(x)` | `ValueExpr::Call { builtin: Builtin::Abs, args }` | Distance from zero, for the rules that bound a signed figure both ways at once - a net position long minus short, against a limit that does not care which side it is on. A real node rather than a desugaring to `max(x, 0 - x)`: that spelling evaluates the operand twice, and a `sum` operand evaluated twice is a bug waiting for a reader who does not notice. Function-shaped, like `min`/`max` and `round`, so it is self-delimiting and adds no precedence tier. |
| `round(x, quantum)` | `ValueExpr::Call { builtin: Builtin::Round, args }` | The multiple of `quantum` nearest to `x`, exact halves AWAY FROM ZERO (`round(0.125, 0.01)` is `0.13`; `round(0 - 0.125, 0.01)` is `-0.13` - the symmetry that keeps credit notes fair). One mode only, measured against a real billing convention; a second policy joins as a parameter when a real bill forces it, never speculatively. Decimal-only in v0: money rounds as a bare decimal under the currency-in-field-names convention, and a quantity mode waits for its example. A non-positive quantum is refused by name - at authoring time when literal, at evaluation otherwise. A dedicated node for the `abs` reasons: the operand evaluates once, the form round-trips as written, and it can never be mistaken for the sign-branched shift-and-remainder spelling it replaces. Function-shaped like `min`/`max`: self-delimiting, no new precedence tier. |
| `if(<prop>, <value>, <value>)` | `ValueExpr::Cond { when, then, otherwise }` | The value selected by whether a proposition holds. The test is exists-style: at least one witness selects the first value, none selects the second, and the witnesses' bindings are DISCARDED - `require`'s non-export rule, so nothing bound inside the condition reaches the branches or the surrounding expression. An error in the condition propagates (an undecidable condition never silently selects the fallback), and ONLY the selected branch evaluates - a division by zero or a defaultless `value` lookup in the untaken branch cannot surface, which is what lets one act serve every source (`if(source = #meter, value MeterReading(meter, _), proposed_qty)`: the caller-sourced case needs no reading at all). Branch kinds unify with no ordering requirement - selection is not ordering, so subject tags and booleans are lawful branch kinds. Function-shaped like `round`: self-delimiting, no precedence tier, no dangling else; `if` is contextual (constructor only before `(`), so it remains a legal variable name. Two boundaries to know: the durable adapter's read footprint is the conservative union of the condition and BOTH branches (branch evaluation is lazy; loading is not), and the condition contributes its whole reference set - present, absent, or under `not` - to the gate-vs-invariant dependency walk, because a retractable pointer read only as a condition still decides what permanent history must satisfy. A kernel node, not sugar: an `or` of tests can only CHECK an already-bound value, never produce one. |
...
| `sum(target | body)` | `ValueExpr::Sum` | Set-builder notation. The target is any value expression consuming the body's bindings, evaluated exactly once per witness: a variable (`sum(amount | body)`), a decimal literal counting matches (`sum(1 | body)` - the chess material census forced this), or a computed quantity (`sum(probability * loss | body)` - the expected-loss recompute forced the generalisation). The target binds nothing and never changes which witnesses exist; an error evaluating it for one witness fails the whole sum. A bare `actor` target is refused by name, and a wildcard anywhere a value must be produced - the whole target, an arithmetic operand, a conditional branch - is refused at authoring time (`WildcardAsValue`). Type-driven: a sum of decimals is a decimal, a sum of durations is a duration (counted laytime forced this), a sum of same-unit quantities is a quantity of that unit (the cargo book forced this); mixing kinds - or units - is an error. The empty sum is the typed zero of the target's declared kinds (`0 t` over a `Decimal[t]` position, `duration(PT0S)` over a duration, decimal zero otherwise), resolved statically by the `lower_sum_seeds` pass - variables from the claim position that binds them, literals from their own kind, `value` lookups from the declared kind of the extracted position, conditionals from branches whose kinds agree, a nested sum from its own seed, arithmetic through the rule matrix (`qty * factor` keeps the unit even with the factor's kind unknown). A target the pass cannot type stays decimal only where that is lawful: when the checker reads a duration or quantity the seed cannot match (an outer-bound variable, a builtin call), the programme is refused at authoring time (`EmptySumUntyped`) rather than handed a bare-decimal zero the first empty book would turn into a kernel error. The extremum target is deliberately still a variable or literal - no worked example has forced a computed extremum. |
...
| `value Pred(args)` (with optional `default expr`) | `ValueExpr::ValueOf` | Claim-pattern form, two spellings for one IR. Positional: the FIRST `_` in `args` is the extraction and every later `_` is an ordinary match-anything filter. Named: exactly one field written `field: _` is the extraction hole and `..` elides the unconstrained coordinates - `value EligibleCollateral(asset: asset, collateral_value: _, ..)` reads the figure without stating the facility, a shape no positional spelling reaches (the first `_` would land on the elided coordinate, not the figure). That is why the kernel's `ValueOf { predicate, args, extract, default }` carries the extraction index explicitly: `args[extract]` must be a wildcard (refused at validation and again at evaluation otherwise), the positional parser computes it as the first wildcard, and the formatter renders positionally EXCEPT when the hole is not the first wildcard, where the named spelling is the only one that reparses to the same IR - forced in every rendering mode, including the canonical-hash rendering. Zero or several `field: _` entries refuse by name. A `value(target \| body)` shape would imply a general query and be more expressive than the IR, so neither spelling grows one. |
| `derived Name(keys):` with an indented `over domain` clause and `value name = expr` clauses | `DerivedClaim { predicate, keys, values, domain }` | A governed read-side view, recomputed from admitted claims on demand - never stored, never admitted, so it cannot drift from the claims it reads. The head lists the KEY fields; enumeration yields one row per distinct key-tuple the `over` domain binds, and each `value` clause computes one output field against that row's bindings. The row is positional: keys, then values, in declaration order. The head is also the projection: it may keep FEWER variables than the domain binds, and enumeration collapses to one row per distinct projected key tuple - a view is keyed by what identifies it, not by every coordinate it joins through. The rules this shape enforces: at least one `value` clause is required (a derived is a computation over a domain, not a stored relation - a value-less "which subjects match" view is a domain query the embedder runs, or an invariant if it must be guaranteed); every non-computed field the row carries must be a key the domain binds; and value expressions see the head-key bindings ONLY - never one another (no `let`-style chaining between clauses), and never a domain variable the head dropped, because values run after witnesses collapse, where a non-key variable has no single value. A domain-bound non-key variable in a value expression is refused at authoring with both remedies named: add it to the head if the value depends on it, or - when it appears only to position a lookup - drop it and name the extracted field, eliding the unused coordinates with `..`. |
...
| `span(P3M)` | `Value::CalendarSpan("P3M")` | The calendar-side twin of `duration(...)`: whole months and whole days, the units whose length depends on the date they land on. The grammar is Morpholog's own, not jiff's - uppercase `P[nY][nM][nD]` or a standalone `PnW`, unsigned whole numbers, at least one component, no time part even zero-valued (`span(PT6H)` is refused with `duration(...)` named as the fix); years normalise to twelve months and weeks to seven days at parse time, and days never normalise into months (`P1M` and `P30D` are different shifts). A span is EXPRESSION-ONLY: it shifts a `Date` inside arithmetic (`ends_on + span(P45D)`; months walk first, the day clamping to the destination month's last day, then days step - so `@2026-11-30 + span(P3M)` is `@2027-02-28`, the walk is neither reversible nor associative around clamped month ends, and repeated `span(P3M)` hops are NOT one `span(P6M)` shift), while date subtraction (`days_late = as_of - deadline`) counts the signed actual days between two dates as an integer-valued decimal - the ACT numerator, so ACT/360-style fractions are plain division. Everything else refuses, at check time and again at every runtime boundary: no span in a claim, intent, collection, or transition argument - even through an `Any`-kinded slot - no declaration carries the kind, no derived value resolves to one, a transformation parameter inferred as a span is refused (no lawful argument could ever fill it), no ordered comparison, sum, or extremum accepts one, spans do not combine with each other, and `Timestamp +/- span` stays a type error (a calendar shift of an instant needs a time zone the kernel refuses to guess). Equality alone is lawful, over the normalised value: `span(P1Y) = span(P12M)` holds, `span(P1M) = span(P30D)` does not. `span` is contextual, like `duration`. |
| `period_index(<anchor>, <span>, <at>)` | `ValueExpr::Call { builtin: Builtin::PeriodIndex, args }` | The integer-valued decimal coordinate of `at` in the anniversary partition defined by `anchor` and a positive calendar span. Boundary n is computed ONCE from the original anchor by shifting it by n-times-months then n-times-days (the standard clamped walk) - boundaries are never produced by repeated hops, whose drift the calendar's non-associativity would compound (a 31 January anchor under monthly spans keeps its 31-March boundary; a leap-day anchor clamps each anniversary from the anchor and RETURNS to 29 February in leap years). Representable boundaries form half-open periods `[B(n), B(n+1))`; a boundary shifted below or above the representable calendar acts as an infinity, so the outermost periods are CLIPPED to the calendar and the extractor is total - exact anniversaries enter the new period, dates before the anchor take negative indexes. Kinds `(Date, CalendarSpan, Date) -> Decimal`; a zero span is refused by name at validation when literal and at evaluation otherwise (the round-quantum two-tier pattern). The operator adds no dependencies beyond its three value children's own (no selection semantics - contrast `if`), and a fully-literal use is lawful in a `const`. It subsumes the comparator spelling (same-period is index equality), annual indexing (`span(P1Y)`), and same-calendar-month membership (a first-of-month anchor under `span(P1M)`); the index is a plain decimal, so a record may carry its period coordinate, kept honest by a recompute invariant. |
| `period_start_of(<anchor>, <span>, <index>)` | `ValueExpr::Call { builtin: Builtin::PeriodStartOf, args }` | The boundary date back from the coordinate: period `index`'s first day, boundary n computed exactly as `period_index` defines it (span components multiplied by n ONCE, one clamped walk from the anchor), so the two forms are inverse spellings of one boundary - `period_index(a, s, period_start_of(a, s, n)) = n` wherever the boundary is representable, and a date is an anniversary exactly when the round trip returns it, under the same proviso: in the clipped lowermost period, whose own starting boundary is unrepresentable, the composed test refuses rather than answering. Deliberately NOT span arithmetic: spans still do not combine and `anchor + n * span` stays inexpressible - only the boundary computation `period_index` already owns is exposed. Kinds `(Date, CalendarSpan, Decimal) -> Date`. The refusal story differs from `period_index`'s clipping ON PURPOSE: a DATE outside every representable boundary still belongs to an outermost period, but an INDEX whose boundary leaves the calendar has no honest date to return, so it raises the same out-of-range refusal a calendar-escaping `date + span` earns. A zero span is refused two-tier as in `period_index`; a fractional index likewise (literal at validation by name, computed at evaluation) - period coordinates are integers, there is no period between periods. Negative indexes name the periods before the anchor. No selection semantics, and a fully-literal use is lawful in a `const`. Forced by the schedule-to-coordinate split: a rate sheet keyed by the day its year begins and a record carrying the year's number are two spellings of one period only if a rule can compute the date back. |
...
6. If any revalidation fails: rollback. Nothing commits. No business audit record.
