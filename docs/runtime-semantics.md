...
| `25000 USD`, `0 t` | `Value::Quantity { amount, unit }` | Amount-then-unit juxtaposition, the way a charterparty or an invoice writes it. The syntax claim: a numeric literal followed by an identifier in term position is a quantity literal. No signed amounts (the surface has no signed decimal literals anywhere); the zero literal exists for the aggregate seed pattern. |
