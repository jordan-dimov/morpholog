...
5. **The `compare` table** for baseline against candidate: `suite --format json` run several times on each side, then `morpholog-bench compare --before a1.json a2.json a3.json a4.json --after b1.json b2.json b3.json b4.json`. It pairs every case metric by case, axis, point, metric and unit, prints the median of each side's runs and their ratio, refuses two rulers, two ladders, or one metric in two units, and lists any row only one side has - so a changed plan cannot pass as a changed number. Each row also says whether the difference is real: `lower` or `higher` only when every run on one side lies beyond every run on the other and chance alone would do that at most one time in twenty, `within noise` when the runs overlap, and `too few runs` when even complete separation would prove nothing. Four runs a side is the least that can show a change. **The run is the unit of evidence, not the repeat:** repeats inside one run share a process, a fixture and a buffer cache, and agree with each other far more closely than two runs of the same binary do. An A/A pair judged on its repeats called about one row in ten changed (see the bench README's observations). A ratio on a `within noise` or `too few runs` row is not a result. Type nothing by hand.
