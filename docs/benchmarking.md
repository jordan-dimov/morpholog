...
5. **The `compare` table** for baseline against candidate: `suite --format json` on each, then `morpholog-bench compare before.json after.json`, which pairs every case metric, prints the ratio, refuses two rulers, and lists any row only one side has - so a changed plan cannot pass as a changed number. Type nothing by hand.
