...
- `write`, `read`, `asof`, `contend` - the original mechanism scenarios; their *complement* cases (`/noise`, `/grouped`, `/retract`, `/shared` vs `/disjoint`, the workers=1 baseline) are deliberate controls, not defaults. They are what let a reader say **why** a number moved, and they are the first thing a narrow optimisation regresses.
