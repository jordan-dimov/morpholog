...
# sqlx query macros verify against the committed `.sqlx/` offline cache,
# so every cargo step below builds with no database. Freshness against
# the live schema is checked separately when DATABASE_URL is set.
