...
The same ruler again after case-local admission landed (#365), with the stage-1 binary (6174bef) and the rung-2b binary (1746237) run back to back in one session. The machine was thermally throttled that day, so every absolute number is roughly twice the previous day's, the read families included; the within-session ratios are the claim and the earlier table is historical context, not a baseline.
...
- **Controls:** read, replay and as-of did not move between the binaries.
