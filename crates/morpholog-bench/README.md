...
- **New finding - `wide/size` is the suite's steepest curve**: a 13-ary predicate under one grouped-sum invariant proposes in ~58ms/2.6s/~7min at 1k/10k/100k rows - super-linear in the predicate's row count (each proposal re-evaluates the group sum over the whole loaded predicate) - while `wide/arity` at fixed 10k rows is nearly flat (~2.8-3.1s from arity 4 to 13). **Row count under an aggregate invariant, not argument count, is the expensive axis.** The 100k cell dominates the full run's wall clock (~35 of its ~75 minutes); that is a finding about the interpreted runtime, recorded rather than trimmed, and it is the sharpest measured pressure yet toward compiled invariant checking, where that sum is a SQL aggregate.
