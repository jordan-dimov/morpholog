...
| `kernel` | an in-memory ledger of N entries; no database, no `--reset` | one `propose` with the ledger's invariants, one with none, and `--acts` sequential proposals each against the candidate the act before it produced | the kernel alone: invariant evaluation against the book versus the candidate build by itself, and the interpreted core of `transact` |
| `replay` | the `as-of` fixture | `coverage_replay` and `score_candidate` of the ledger against itself | the verification arc: what a replay that observes every audit row costs as the log grows |

`write` also reports where each proposal's time went - `phase_begin_tx`, `phase_load_state`, `phase_kernel`, `phase_finalise` - read from the production path, which records four clock readings per proposal. That split is what tells a rebuild from an invariant scan from a commit before anyone estimates.
