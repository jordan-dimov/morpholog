...
/// [`all_examples`] registry is the single authority on what "all
/// worked examples" means: `all_programs()` derives from it, and
/// consumers that need the file rather than the programme (the CLI's
/// all-examples check) iterate the same descriptors instead of
/// re-walking the filesystem with their own heuristic.
#[derive(Debug, Clone)]
...
// The auto-discovered registry (generated by build.rs):
// `all_examples()` plus the `all_programs()` derived from it.
