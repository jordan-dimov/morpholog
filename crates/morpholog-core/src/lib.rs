...
mod tests {
    use super::*;
    use crate::examples::{claim_standing, revenue_restatement, settlement_netting};
    ...
    // -----------------------------------------------------------------
    // Claim standing (Example 3) — admissibility-for-purpose
    //
    ...
    // -----------------------------------------------------------------
    // JSON codec — runtime values, claims, intents
    //
    // PG persistence will write these JSON shapes into JSONB columns
    // declared in `crates/morpholog-core/sql/schema.sql`. The round-trip
    // tests pin both the wire shape (so future readers cannot accidentally
    // change it) and the exactness contract (decimals serialise as JSON
